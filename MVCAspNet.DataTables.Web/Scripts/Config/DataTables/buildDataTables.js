...
/** Como utilizar:
     Basicamente para utilizar é necessário que a Table(<table>) tenha a classe "_tbDataTables". Essa table têm que ter a separação de <thead> <tbody> no mínimo!
     ** Forma de atribuir o Plugin:
      * Básica: 
          Não atribuindo nenhum atribudo para sua table, o função para carregar (PageDataTables.Load()) vai entender que é para atribuir o plugin depois
          que a table já foi preechida.
              Ex.: <table class="_tbDataTables"><thead><tr><th>RA</th><th>Nome do Aluno</th></tr></thead><tbody></tbody></table>
      * Carregamento Ajax somente no Inicio:
          Atribuindo na sua tabela (<table>) o ajaxsource (WebService do Ajax) com o caminho do WebService o destroy como "true" o processing como "true".
          No seu <thead> nos <th> o column com o nome do atributo que vai vindo no json do WebService consumido.
              Ex.: <table class="_tbDataTables" ajaxsource="Aluno/ProcessarAoIniciar" destroy="true" processing="true">
                     <thead><tr><th column="nr_ra_aluno">RA</th><th column="nm_aluno">Nome do Aluno</th></tr></thead>
                     <tbody></tbody>
                   </table>
      * Processamento 100% Ajax:
          Atribundo na sua tabela (<table>) o ajaxsource (WebService do Ajax) com o caminho do WebService e o atribudo serverside como true.  No seu <thead> 
          nos <th> o column com o nome do atributo que vai vindo no json do WebService consumido.
              Ex.: <table class="_tbDataTables" ajaxsource="Aluno/ProcessarDataTables">
                     <thead><tr><th column="nr_ra_aluno">RA</th><th column="nm_aluno">Nome do Aluno</th></tr></thead>
                     <tbody></tbody>
                   </table>
     ** Atributos:
       Como descrito acima, para utilizar esse .js é necessário colocar atributos na tabela (<table>), tentei pegar os principais na função de 
       carregar (PageDataTables.Load()), mas nada impede de implementar mais no decorrer de novas necessidade!
       Os atributos que ela procura na tabela <table>:
       ...
       Os atributos que ela procura no Head (<thead><tr><th>) da tabela (<table>):
